...
func liveCloneFloat64(value *float64) *float64 {
	if value == nil {
		return nil
	}
	copy := *value
...
func (s *OpenAIGatewayService) calculateLiveCallCost(durationMs int, record *LiveCallRecord) *CostBreakdown {
	if durationMs <= 0 {
	...
	return billingService.CalculateAudioCost("realtime", minutes, &audioPriceConfig{
		RealtimePerMin: record.AudioRealtimePricePerMin,
	}, liveMultiplierOrDefault(record.RateMultiplier))
}

func (s *OpenAIGatewayService) applyLiveUsageBilling(ctx context.Context, record *LiveCallRecord, usageLog *UsageLog, cost *CostBreakdown) error {
	if record == nil || usageLog == nil || cost == nil || cost.ActualCost <= 0 {
		return nil
	}
	if s.cfg != nil && s.cfg.RunMode == config.RunModeSimple {
		if s.deferredService != nil {
			s.deferredService.ScheduleLastUsedUpdate(record.AccountID)
		}
		return nil
	}
	if s.usageBillingRepo == nil && !s.hasLegacyLiveBillingDeps(record) {
	...
	account := &Account{
		...
		RateMultiplier: liveCloneFloat64(record.AccountRateMultiplier),
	}
	...
	_, err := applyUsageBilling(ctx, record.CallHash, usageLog, &postUsageBillingParams{
		...
		AccountRateMultiplier: liveMultiplierOrDefault(record.AccountRateMultiplier),
		Platform:              record.Platform,
...
func (s *OpenAIGatewayService) hasLegacyLiveBillingDeps(record *LiveCallRecord) bool {
	if record != nil && record.SubscriptionID > 0 {
		return s.userSubRepo != nil
	}
	return s.userRepo != nil
}

func (s *OpenAIGatewayService) liveBillingDeps() *billingDeps {
	deps := s.billingDeps()
	deps.cfg = s.cfg
	if deps.deferredService == nil {
		deps.deferredService = NewDeferredService(nil, nil, 0)
	}
	return deps
}
...
func (s *OpenAIGatewayService) CreateLiveCall(
	...
	for attempt := 0; attempt <= 3; attempt++ {
		...
		record := &LiveCallRecord{
			...
			RateMultiplier:           liveCloneFloat64(identity.RateMultiplier),
			AudioRealtimePricePerMin: liveCloneFloat64(identity.AudioRealtimePricePerMin),
			LeaseID:                  leaseID,
			Model:                    model,
			CreatedAt:                now,
...
func (s *OpenAIGatewayService) finalizeLiveCall(record *LiveCallRecord) {
	...
	cost := s.calculateLiveCallCost(duration, record)
	rateMultiplier := liveMultiplierOrDefault(record.RateMultiplier)
	var billingMode *string
	if cost != nil && cost.BillingMode != "" {
		value := cost.BillingMode
		billingMode = &value
	}
	usageLog := &UsageLog{
		UserID:                record.UserID,
		...
		AccountRateMultiplier: liveCloneFloat64(record.AccountRateMultiplier),
	...
	if err := s.applyLiveUsageBilling(context.Background(), record, usageLog, cost); err != nil {
		logger.LegacyPrintf("service.openai_live", "live billing failed call_hash=%s user=%d account=%d cost=%f: %v", record.CallHash, record.UserID, record.AccountID, cost.ActualCost, err)
