...
func (s *GatewayService) ForwardAsChatCompletions(
	...
	if err := s.ensureClaudeRequestPricing(ctx, account, originalModel, mappedModel); err != nil {
		writeChatCompletionsError(c, http.StatusInternalServerError, "pricing_error", "Model pricing is unavailable; contact the administrator")
		return nil, err
	}
...
func (s *GatewayService) handleCCStreamingFromAnthropic(
	...
	if err := s.consumeAnthropicCompatStream(resp, c, true, processAnthropicEvent); err != nil {
		return resultWithUsage(), writeAnthropicCompatStreamFailure(c, err, false, clientDisconnected)
	}
