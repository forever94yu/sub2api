...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Equal(t, schedulerTestAccountQueryCount()*3, accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, schedulerTestCanonicalBucketCount(), reopenCount)
		require.Equal(t, schedulerTestCanonicalBucketCount()+1, capturesAtFirstDB, "C(0) and the historical bucket must be captured before DB")
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
	...
	require.Equal(t, schedulerTestAccountQueryCount()*2+1, accounts.callCount())
	require.Equal(t, schedulerTestAccountQueryCount()+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildOrdinaryCaptureErrorReturnsBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, schedulerTestCanonicalBucketCount()+2, cache.captureAttemptCount(), "all canonical and ordinary captures must be attempted before returning")
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, schedulerTestCanonicalBucketCount()*2+3, cache.captureAttemptCount())
	require.Equal(t, schedulerTestAccountQueryCount()*2+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, schedulerTestAccountQueryCount(), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	groupRetirementCount := schedulerTestCanonicalBucketCount() + 1
	require.Equal(t, groupRetirementCount, len(cache.retiredBuckets()))
	...
	require.Equal(t, groupRetirementCount*3, len(cache.retiredBuckets()))
	require.Equal(t, schedulerTestAccountQueryCount(), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildActiveTombstoneLazyRecoveryDiscardsPartialCaptureTasks(t *testing.T) {
	...
	accounts.beforeFirst = func() {
		...
		require.Equal(t, schedulerTestCanonicalBucketCount()+retiredBucketIndex+2, capturesAtFirstDB)
	}
	svc := newFullRebuildLifecycleService(cache, nil, accounts, groups, config.RunModeStandard)
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Zero(t, fallbackCalls)
	require.Empty(t, freshCalls)
	require.Equal(t, schedulerTestCanonicalBucketCount()+len(registered), cache.captureAttemptCount())
	require.Equal(t, schedulerTestAccountQueryCount()+len(registered), accounts.callCount())
	require.Equal(t, schedulerTestAccountQueryCount()+len(registered), accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, schedulerTestAccountQueryCount()*2, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, schedulerTestAccountQueryCount()*4, accounts.callCount())
