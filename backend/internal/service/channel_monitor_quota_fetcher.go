...
// 渠道监控「配额模式」的配额抓取器。
//
// 不直接对接上游，而是把账号侧现成的 AccountUsageService 用量结果归一成
...
//
// 多个监控可能关联同一账号，而 interval 最小 15s，所以快照统一带 TTL 缓存（成功 monitorQuotaFetchCacheTTL、失败
// monitorQuotaErrorCacheTTL 负缓存），防止打爆上游配额端点；同账号的并发
// 抓取由 singleflight 合并为一次上游查询。

// monitorUsageSource 平台账号用量查询（AccountUsageService 天然满足）。
...
func (f *ChannelMonitorQuotaFetcher) Fetch(ctx context.Context, accountID int64) *domain.MonitorQuotaSnapshot {
	...
	// singleflight 合并同账号并发抓取；脱离调用方 ctx，
...
// fetchUsage 将 AccountUsageService.GetUsageForAccount 结果转换为快照。
...
// quotaDegradedHint 生成 degraded 的 message（指出触发告警的窗口）；
// 空串表示无告警。
