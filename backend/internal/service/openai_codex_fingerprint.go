...
const (
	...
	// 并固定 turn metadata / prompt_cache_key 等会话指纹载体。
	// 上游看到 1 台设备 + 1 会话 + 1 套账号级指纹，最激进。
...
//
// 未设置、空值或非法值默认按 full 处理；管理员仍可显式选择 off/device/session。
...
func resolveConvergedTurnID(seed string) string {
	if seed == "" {
		return ""
	}
	return deriveStableUUIDv4("sub2api:codex-turn-id:v2:" + seed)
}

func resolveConvergedTurnStartedAtUnixMs(seed string) int64 {
	if seed == "" {
		return 0
	}
	h := sha256.Sum256([]byte("sub2api:codex-turn-started-at:v2:" + seed))
	const (
		stableBaseUnixMs = int64(1704067200000) // 2024-01-01T00:00:00Z
		stableWindowMs   = uint64(366 * 24 * 60 * 60 * 1000)
	)
	return stableBaseUnixMs + int64(binary.BigEndian.Uint64(h[:8])%stableWindowMs)
}

// codexFingerprintIDs 收敛后的完整 ID 集合。
// 由 resolveCodexFingerprintIDs 一次性生成，同一个实例在头改写和体改写之间共享，
// 确保所有载体中的 turn_id 等字段一致。体改写时还会补记原始
...
// 注意：session 模式包含随机生成的 turn_id，调用方必须只调用一次并共享结果给
// 头改写和体改写；full 模式下 turn metadata 也从账号种子确定性派生。
func resolveCodexFingerprintIDs(account *Account, clientSessionID string, mode codexFingerprintMode) *codexFingerprintIDs {
	...
	case codexFingerprintFull:
		ids.sessionID = resolveConvergedSessionID(seed)
		ids.threadID = ids.sessionID
		ids.turnID = resolveConvergedTurnID(seed)
		ids.turnStartedAtUnixMs = resolveConvergedTurnStartedAtUnixMs(seed)
		ids.windowID = ids.threadID + ":0"
		return ids
	}
...
// 做标量改写；full 模式会收敛任意非空字符串 prompt_cache_key。语义与
// applyCodexFingerprintClientMetadata 逐点一致（含
// "非对象值整体替换为收敛集合"的行为）。
