...
func TestResolveCodexFingerprintIDs_FullModeConvergesAllSessionFingerprintFields(t *testing.T) {
	account := newTestOAuthAccount(1, map[string]any{
		codexFingerprintModeExtraKey: "full",
	})

	clientA := http.Header{}
	clientA.Set("session-id", "client-session-A")
	idsA := resolveCodexFingerprintIDsFromRequest(account, clientA)
	require.NotNil(t, idsA)

	clientB := http.Header{}
	clientB.Set("session-id", "client-session-B")
	idsB := resolveCodexFingerprintIDsFromRequest(account, clientB)
	...
	assert.Equal(t, idsA.turnID, idsB.turnID)
	assert.Equal(t, idsA.windowID, idsB.windowID)
	assert.Equal(t, idsA.turnStartedAtUnixMs, idsB.turnStartedAtUnixMs)
}
...
func TestApplyCodexFingerprintPromptCacheKey_MapRawEquivalence(t *testing.T) {
	...
	t.Run("full converges explicit override", func(t *testing.T) {
		account := newTestOAuthAccount(4302, map[string]any{codexFingerprintModeExtraKey: "full"})
		ids := resolveCodexFingerprintIDs(account, "header-session", codexFingerprintFull)
		require.NotNil(t, ids)

		body := []byte(`{"model":"gpt-5.6-sol","prompt_cache_key":"explicit-cache-A","client_metadata":{"session_id":"body-session"},"input":[]}`)
		mapBody, rawBody := applyMapAndRawFingerprintBodiesForTest(t, body, ids)
