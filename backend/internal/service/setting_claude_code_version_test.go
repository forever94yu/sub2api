...
type claudeVersionConcurrentReadRepo struct {
	SettingRepository
	reads   atomic.Int32
	started chan struct{}
	release chan struct{}
...
func (r *claudeVersionConcurrentReadRepo) GetMultiple(context.Context, []string) (map[string]string, error) {
	version := "2.1.281"
	if r.reads.Add(1) == 1 {
		version = "2.1.280"
		close(r.started)
	...
	return map[string]string{SettingKeyClaudeCodeClientVersion: version}, nil
}

func TestGetClaudeCodeClientVersionInvalidationDuringRead(t *testing.T) {
	repo := &claudeVersionConcurrentReadRepo{started: make(chan struct{}), release: make(chan struct{})}
	t.Cleanup(func() {
		select {
		case <-repo.release:
		default:
	...
	go func() { oldResult <- svc.GetClaudeCodeClientVersion(context.Background()) }()
	select {
	case <-repo.started:
	case <-time.After(5 * time.Second):
		t.Fatal("old version lookup did not start")
	}

	svc.InvalidateClaudeCodeClientVersionCache()
	require.Equal(t, "2.1.281", svc.GetClaudeCodeClientVersion(context.Background()))
	close(repo.release)
	select {
	case <-oldResult:
	case <-time.After(5 * time.Second):
		t.Fatal("old version lookup did not finish")
	}
	require.Equal(t, "2.1.281", svc.GetClaudeCodeClientVersion(context.Background()), "an old in-flight read must not repopulate the cache after settings are saved")
}
