...
func (h *RedeemHandler) GetStats(c *gin.Context) {
	type redeemStatsProvider interface {
		GetRedeemCodeStats(context.Context) (*service.RedeemCodeStats, error)
	}
	if provider, ok := h.adminService.(redeemStatsProvider); ok {
		stats, err := provider.GetRedeemCodeStats(c.Request.Context())
		if err != nil {
			response.ErrorFrom(c, err)
			return
		}
	...
	response.Success(c, (&service.RedeemCodeStats{
