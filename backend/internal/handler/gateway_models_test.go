...
func TestGatewayModels_AnthropicCustomModelsListIncludesOAuthClaudeAndMappedGeneric(t *testing.T) {
	gin.SetMode(gin.TestMode)
			...
			byGroup: map[int64][]service.Account{
				groupID: {
					...
					},
					{
						...
						Credentials: map[string]any{
							"model_mapping": map[string]any{
								"vendor-reasoning-v1": "vendor-reasoning-v1",
							},
						},
					},
				},
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"claude-fable-5", "claude-opus-4-8", "vendor-reasoning-v1"},
			},
		},
	})
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"claude-fable-5", "claude-opus-4-8", "vendor-reasoning-v1"}, modelIDsForTest(got.Data))
}
...
func TestGatewayModels_AnthropicCustomModelsListDisabledKeepsMappedModelList(t *testing.T) {
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: false,
				Models:  []string{"claude-fable-5", "vendor-reasoning-v1"},
			},
		},
	})
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"vendor-reasoning-v1"}, modelIDsForTest(got.Data))
}
