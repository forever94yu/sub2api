...
func TestOpenAICompatibleTextTargetAllowsGrok(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestRemovedProviderModelsDoNotResolveCompositeTarget(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	for _, model := range []string{"kimi-k2-thinking", "glm-5.2", "deepseek-v3.2"} {
		...
		require.False(t, openAICompatibleTextTargetAllowed(c, apiKey, model), "model=%s", model)
