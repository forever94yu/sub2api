...
// it; without the restore, reasoning mode rejects the history with 400
...
// Generic emits reasoning only once per turn; chained tool calls
...
// reasoning mode 400s the history ("reasoning_content ... must be
...
func TestResponsesToChat_ChainedToolCallsReplayTurnReasoning(t *testing.T) {
	...
	// 每一条 assistant 消息都必须带 reasoning_content（Generic 契约）。
