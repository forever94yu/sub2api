...
func TestChatCompletionsResponseToResponses_ReasoningOnlyFallsBackToMessageText(t *testing.T) {
	content := json.RawMessage(`""`)
	resp := &ChatCompletionsResponse{
		ID:     "chatcmpl_vendor_reasoning_only",
	...
	out := ChatCompletionsResponseToResponses(resp, "vendor-reasoner", nil, false, nil)
...
func TestChatCompletionsResponseToResponses_ReasoningToolCallDoesNotFallbackToMessageText(t *testing.T) {
	content := json.RawMessage(`""`)
	resp := &ChatCompletionsResponse{
		ID:     "chatcmpl_vendor_reasoning_tool",
		Object: "chat.completion",
		Model:  "vendor-reasoner",
		Choices: []ChatChoice{{
			Index: 0,
